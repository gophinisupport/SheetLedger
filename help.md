...
To protect your privacy and data security, SheetLedger only requests the minimum necessary scope (drive.file), which limits the app to accessing spreadsheets you explicitly authorize. Selecting a sheet shared by someone else relies on the official Google Picker API, which requires browser-based authentication. As a result, SheetLedger will open your external web browser to let you securely log in and pick the shared spreadsheet.
